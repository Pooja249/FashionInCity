    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try
            {
                ...
                PreparedStatement ps=cn.prepareStatement("update subcategorytable set subCategoryName=?,subCategoryStatus=? where subCategoryId=?");
                ps.setString(1, subCategoryName);
                ps.setString(2, subCategoryStatus);
                ps.setString(3, subCategoryId);
                int i=ps.executeUpdate();
                if(i>0)
                {
                    out.println("<script type=\"text/javascript\">");
                out.println("alert('SubCategory Updated Successfully!!')");
